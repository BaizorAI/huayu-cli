    ...
    match event_type {
        "assistant" => {
            ...
            let events = content
                ...
                .map(|l| { let s = l.to_string(); ToolEvent::Line(s) })
                .collect();
            Some(events)
        }
        "tool_use" => {
...
pub fn parse_event(line: &str) -> ToolEvent {
    ...
    // Codex (non-stream-json fallback): never emits interactive prompts —
    // codex exec mode is non-interactive. Skip is_prompt_line.
    // Claude (stream-json): handled in try_parse_stream_json above.
